...
def convert_to_square(image, new_size=None, padding=1):
    ...
    square_image[
        top + padding : top + h + padding, left + padding : left + w + padding
    ] = image.copy()
    ...
    if not new_size is None:
        final_image = cv2.resize(
            final_image, dsize=(new_size, new_size), interpolation=cv2.INTER_CUBIC
        )
    return final_image
...
def get_layerwise_predictions(layerwise_nodes, logits):
    ...
    for i in range(number_of_layers):
        ...
        layerwise_pred.append(torch.nn.Softmax(dim=-1)(layerwise_logit))
        cnt += num_nodes
...
class ModelHandler(object):
    ...
    def initialize(self, context):
        ...
        if self.method == "level":
            if "height_of_tree" in list(self.mapping.keys()):
                self.height_of_tree = int(self.mapping.pop("height_of_tree"))
            else:
                logging.error(
            ...
            if "new_n_classes" in list(self.mapping.keys()):
                self.new_n_classes = int(self.mapping.pop("new_n_classes"))
            else:
                logging.error(
            ...
            if "ete_hierarchy" in list(self.mapping.keys()):
                self.ete_hierarchy = Tree(self.mapping.pop("ete_hierarchy"), format=3)
        ...
        logger.debug("Model file %s loaded successfully", model_pth_path)
    ...
    def postprocess(self, data):
        ...
        if self.method == "level":
            ...
            layerwise_pred = get_layerwise_predictions(layerwise_nodes, data)
