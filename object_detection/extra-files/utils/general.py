...
def labels_to_class_weights(labels, nc=80):
    ...
    labels = np.concatenate(
        labels, 0
    )  # labels.shape = (866643, 5) for COCO, labels.shape = (866643, 7) for rotate COCO
    classes = labels[:, 0].astype(
        np.int
    )  # labels = [class xywh], labels = [class, xywhrm] for rotate
...
def polygon_segment2box(segment, width=640, height=640):
    ...
    polygon_box = np.array(
        multipoint.minimum_rotated_rectangle.exterior.coords[:-1]
    ).ravel()
...
def polygon_box_iou(
    boxes1, boxes2, GIoU=False, DIoU=False, CIoU=False, eps=1e-7, device="cpu"
):
    """
    ...
    if torch.cuda.is_available() and polygon_inter_union_cuda_enable and boxes1.is_cuda:
        ...
        inter, union = polygon_inter_union_cuda(
            boxes2_, boxes1_
        )  # Careful that order should be: boxes2_, boxes1_.
    else:
    ...
    if GIoU or DIoU or CIoU:
        ...
        b1_x1, b1_x2 = (
            boxes1[:, 0::2].min(dim=1)[0],
            boxes1[:, 0::2].max(dim=1)[0],
        )  # 1xn
        b1_y1, b1_y2 = (
            boxes1[:, 1::2].min(dim=1)[0],
            boxes1[:, 1::2].max(dim=1)[0],
        )  # 1xn
        b2_x1, b2_x2 = (
            boxes2[:, 0::2].min(dim=1)[0],
            boxes2[:, 0::2].max(dim=1)[0],
        )  # 1xm
        b2_y1, b2_y2 = (
            boxes2[:, 1::2].min(dim=1)[0],
            boxes2[:, 1::2].max(dim=1)[0],
        )  # 1xm
        for i in range(boxes1.shape[0]):
            cw = torch.max(b1_x2[i], b2_x2) - torch.min(
                b1_x1[i], b2_x1
            )  # convex (smallest enclosing box) width
            ch = torch.max(b1_y2[i], b2_y2) - torch.min(
                b1_y1[i], b2_y1
            )  # convex height
            if (
                CIoU or DIoU
                ...
                c2 = cw**2 + ch**2 + eps  # convex diagonal squared
                rho2 = (
                    (b2_x1 + b2_x2 - b1_x1[i] - b1_x2[i]) ** 2
                    + (b2_y1 + b2_y2 - b1_y1[i] - b1_y2[i]) ** 2
                ) / 4  # center distance squared
                if DIoU:
                    iou[i, :] -= rho2 / c2  # DIoU
                elif (
...
def polygon_bbox_iou(
    ...
    if (
        ...
        and polygon_b_inter_union_cuda_enable
        and boxes1.is_cuda
    ):
        # using cuda extension to compute
        ...
        boxes1_ = boxes1.float().contiguous().view(-1)
        boxes2_ = boxes2.float().contiguous().view(-1)
        inter, union = polygon_b_inter_union_cuda(
            boxes2_, boxes1_
        )  # Careful that order should be: boxes2_, boxes1_.
    else:
        # using shapely (cpu) to compute
...
def xywhrm2xyxyxyxy(xywhrm):
    ...
    xyxyxyxy = (
        torch.cat((x0, y0, x1, y0, x1, y1, x0, y1), dim=-1).view(-1, 4, 2).contiguous()
    )
    R = torch.zeros(
...
def xyxyxyxy2xywhrm(xyxyxyxy):
    ...
    xyxyxyxy = (
        order_corners(torch.from_numpy(xyxyxyxy))
        if is_array
        else order_corners(xyxyxyxy.clone())
    )

    w = torch.max(
        (
            (xyxyxyxy[:, 2] - xyxyxyxy[:, 0]) ** 2
            + (xyxyxyxy[:, 3] - xyxyxyxy[:, 1]) ** 2
        ).sqrt(),
        (
            (xyxyxyxy[:, 6] - xyxyxyxy[:, 4]) ** 2
            + (xyxyxyxy[:, 7] - xyxyxyxy[:, 5]) ** 2
        ).sqrt(),
    )
    h = torch.max(
        (
            (xyxyxyxy[:, 4] - xyxyxyxy[:, 2]) ** 2
            + (xyxyxyxy[:, 5] - xyxyxyxy[:, 3]) ** 2
        ).sqrt(),
        (
            (xyxyxyxy[:, 6] - xyxyxyxy[:, 0]) ** 2
            + (xyxyxyxy[:, 7] - xyxyxyxy[:, 1]) ** 2
        ).sqrt(),
    )
...
def rotate_segments2boxes(segments, img_shapes=None):
    ...
    for segment, img_shape in zip(segments, img_shapes):
        polygon_box = (
            polygon_segment2box(segment)
            if img_shape is None
            else polygon_segment2box(segment, img_shape[1], img_shape[0])
        )
...
def rotate_box_iou(
    ...
    return polygon_box_iou(
        boxes1_xyxyxyxy, boxes2_xyxyxyxy, GIoU, DIoU, CIoU, eps, device
    )  # IoU
...
def rotate_bbox_iou(
    ...
    boxes1_xyxyxyxy = xywhrm2xyxyxyxy(boxes1)
    boxes2_xyxyxyxy = xywhrm2xyxyxyxy(boxes2)
    return polygon_bbox_iou(
        boxes1_xyxyxyxy, boxes2_xyxyxyxy, GIoU, DIoU, CIoU, eps, device
    )  # IoU


def rotate_non_max_suppression(
    prediction,
    conf_thres=0.25,
    iou_thres=0.45,
    classes=None,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if merge and (1 < n < 3e3):  # Merge NMS (boxes merged using weighted mean)
            ...
            iou = (
                rotate_box_iou(boxes[i], boxes, device=prediction.device) > iou_thres
            )  # iou matrix
            weights = iou * x[:, 6][None]  # rotated box weights
            x[i, :6] = torch.mm(weights, x[:, :6]).float() / weights.sum(
                1, keepdim=True
            )  # merged rotated boxes
            if redundant:
                i = i[iou.sum(1) > 1]  # require redundancy
