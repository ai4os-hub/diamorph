        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                real, imagin = (
                    ps[:, 4].sigmoid(),
                    ps[:, 5].tanh(),
                )  # real and imaginary parts for rotated angles
                pbox = torch.cat(
                    (pxy, pwh, real[:, None], imagin[:, None]), 1
                )  # predicted box
                iou = rotate_bbox_iou(
                    pbox, tbox[i], CIoU=True, device=device
                )  # iou(prediction, target)
                ...
                lbox += (
                    (1 - real**2 - imagin**2) ** 2
                ).mean()  # real and imaginary parts constraints
