...
def polygon_plot_one_box(x, im, color=(128, 128, 128), label=None, line_thickness=3):
    ...
    c = (
        x.cpu().numpy().reshape(-1, 1, 2).astype(np.int32)
        if isinstance(x, torch.Tensor)
        else np.array(x).reshape(-1, 1, 2).astype(np.int32)
    )
    cv2.polylines(
        im, pts=[c], isClosed=True, color=color, thickness=tl, lineType=cv2.LINE_AA
    )
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
...
def polygon_plot_one_box_PIL(
    x, im, color=(128, 128, 128), label=None, line_thickness=None
):
    # Plots one bounding box on image 'im' using PIL
    im = Image.fromarray(im)
    draw = ImageDraw.Draw(im)
    line_thickness = line_thickness or max(int(min(im.size) / 200), 2)
    box = (
        x.cpu().numpy().ravel().astype(np.int32)
        if isinstance(x, torch.Tensor)
        else np.array(x).ravel().astype(np.int32)
    )
    ...
    if label:
        ...
        draw.rectangle(
            [c[0], c[1] - txt_height + 4, c[0] + txt_width, c[1]], fill=color
        )
        draw.text((c[0], c[1] - txt_height + 1), label, fill=(255, 255, 255), font=font)
    return np.asarray(im)


def polygon_plot_images(
    images,
    targets,
    paths=None,
    fname="images.jpg",
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            # check for confidence presence (label vs pred)
            conf = None if labels else image_targets[:, 10]

            if boxes.shape[1]:
                if boxes.max() <= 2.0:  # if normalized with tolerance 1.0
                    boxes[0::2] *= w  # scale to pixels
                    boxes[1::2] *= h
                elif scale_factor < 1:  # absolute coords need scale if image scales
                    boxes *= scale_factor
...
def rotate_plot_one_box_PIL(
    box, im, color=(128, 128, 128), label=None, line_thickness=None
):
    # Plots one bounding box on image 'im' using PIL
...
def rotate_plot_images(
    ...
    # Plot image grid with labels
    targets_xyxyxyxy = (
        torch.zeros((targets.shape[0], 10), dtype=targets.dtype)
        if isinstance(targets, torch.Tensor)
        else np.zeros((targets.shape[0], 10), dtype=targets.dtype)
    )
    targets_xyxyxyxy[:, :2] = targets[:, :2]
    targets_xyxyxyxy[:, 2:] = xywhrm2xyxyxyxy(targets[:, 2:])
    return polygon_plot_images(
        images, targets_xyxyxyxy, paths, fname, names, max_size, max_subplots
    )
...
def rotate_plot_labels(labels, names=(), save_dir=Path(""), loggers=None):
    ...
    nc = int(c.max() + 1)  # number of classes
    x = pd.DataFrame(
        b.transpose(), columns=["x", "y", "width", "height", "cosine", "sine"]
    )
...
def rotate_plot_crops(xywh, real, imagin, im0s, it_obj, save_path=False):
    ...
    # sum              (vectorized c loop, fastest)
    s = np.sum(rotatingimage, axis=2)
    non_black = s != 0  # comparison       (vectorized c loop, fast)
    x, y = np.where(non_black)  # scan & convert   (c loop, slower)
    crop_img_new = rotatingimage[x.min() + 1 : x.max(), y.min() + 1 : y.max()]
    ...
    [xc, yc, w, h] = [k.item() for k in xywh]
