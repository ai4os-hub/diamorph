...
def create_dataloader(
    ...
    with torch_distributed_zero_first(rank):
        custom_dataset_class = (
            Rotate_LoadImagesAndLabels if rotate else LoadImagesAndLabels
        )  # ROTATE
    ...
    dataloader = loader(
        ...
        sampler=sampler,
        pin_memory=True,
        collate_fn=custom_dataset_class.collate_fn4
        if (quad and not rotate)
        else custom_dataset_class.collate_fn,
    )
...
def polygon_random_perspective(
    img,
    targets=(),
    segments=(),
    degrees=10,
    ...
    if n:
        ...
        xy = xy @ M.T  # transform
        new = (xy[:, :2] / xy[:, 2:3] if perspective else xy[:, :2]).reshape(
            n, 8
        )  # perspective rescale or affine
        ...
        if not mosaic:
            ...
            R2[:2] = cv2.getRotationMatrix2D(angle=0.0, center=(0, 0), scale=r)
            M2 = (
                T @ S @ R @ R2 @ P @ C
            )  # order of operations (right to left) is IMPORTANT
            if (
                (border[0] != 0) or (border[1] != 0) or (M2 != np.eye(3)).any()
            ):  # image changed
                if perspective:
                    img = cv2.warpPerspective(
                        img, M2, dsize=(width, height), borderValue=(114, 114, 114)
                    )
                else:  # affine
                    img = cv2.warpAffine(
                        img, M2[:2], dsize=(width, height), borderValue=(114, 114, 114)
                    )
        ...
        new[(cx > width) | (cx < -0.0) | (cy > height) | (cy < -0.0)] = 0.0
...
def polygon_box_candidates(box1, box2, wh_thr=3, ar_thr=20, area_thr=0.1, eps=1e-16):
    ...
    Compute candidate boxes: box1 before augment, box2 after augment, wh_thr (pixels), aspect_ratio_thr, area_ratio
    """
    w1, h1 = box1[0::2].max(axis=0) - box1[0::2].min(axis=0), box1[1::2].max(
        axis=0
    ) - box1[1::2].min(axis=0)
    w2, h2 = box2[0::2].max(axis=0) - box2[0::2].min(axis=0), box2[1::2].max(
        axis=0
    ) - box2[1::2].min(axis=0)
    ar = np.maximum(w2 / (h2 + eps), h2 / (w2 + eps))  # aspect ratio
    return (
...
def rotate_random_perspective(
    ...
    img, targets_xyxyxyxy = polygon_random_perspective(
        img,
        targets_xyxyxyxy,
        segments,
        degrees,
...
class Rotate_LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            # Augment imagespace
            if not mosaic:
                img, labels = rotate_random_perspective(
                    img,
                    labels,
                    degrees=hyp["degrees"],
                    translate=hyp["translate"],
...
def rotate_load_mosaic(self, index):
    ...
    for x in (labels4[:, 1:3], *segments4):
        np.clip(
            x, 0, 2 * s, out=x
        )  # inplace clip when using rotate_random_perspective()

    # Augment
    img4, labels4 = rotate_random_perspective(
        img4,
        labels4,
        segments4,
        degrees=self.hyp["degrees"],
...
def rotate_load_mosaic9(self, index):
    ...
    # Augment
    img9, labels9 = rotate_random_perspective(
        img9,
        labels9,
        segments9,
        degrees=self.hyp["degrees"],
