...
def to_img_pil(img_open_cv):
    return Image.fromarray(
        cv2.cvtColor(img_open_cv, cv2.COLOR_BGR2RGB)
    )
...
class ModelHandler(object):
    ...
    def initialize(
        ...
        modelFile: directory containing the model files
        model_filename: the checkpoint file name (.pth)
        limit_max_image_pixels: if False, then Image.MAX_IMAGE_PIXELS = None
        detect_param_path: path to the detect_params.yaml file 
        """
        with open(
            detect_param_path
        ) as f:
        ...
        model_pt_path = os.path.join(model_dir, model_filename)
    ...
    def preprocess(self, data):
        ...
        logging.info(
            f"Output of preprocess is a tensor of shape {images_tensor.shape}"
        )
    ...
    def postprocess(self, inference_output, data_rgb):
        ...
        cls_inf = inference.ModelHandler()
        cls_inf.initialize(
            model_dir="/home/se1131/diamorph/classification/best_model",
            serialized_file="best_model.tjm",
        )
            ...
            if len(det):
                ...
                for *xywh, real, imagin, conf, cls in reversed(det):
                    ...
                    crop = rotate_plot_crops(
                        xywh, real, imagin, data_rgb[i], it_obj
                    )
                    ...
                    try:
                        ...
                        #    data=byte_im,
                        # ).json()  # github api
                        response = cls_inf.handle([byte_im])
                    ...
                    if (
                        ...
                        class_classif = list(response[0].keys())[0]
                        class_classif_conf = list(
                            response[0].values()
                        )[0]
...
if __name__ == "__main__":
    detection_model_handeler = ModelHandler()
    detection_model_handeler.initialize(
        model_dir="/home/se1131/diamorph/object_detection/best_model",
        detect_param_path= '/home/se1131/diamorph/object_detection/detect_params.yaml',
        modelFile=None,
        model_filename="best_model.pt",
    )
    detections = detection_model_handeler.handle(
        ["/home/se1131/diamorph/object_detection/43.png"]
    )
    print("detections successfully done", detections)
